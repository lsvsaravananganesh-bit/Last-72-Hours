...
function render(){
 updateCycloneIntelligence();renderObjectives();renderCommandLog();renderAchievements();updateRisk();renderForecast();$("clock").textContent=String(Math.max(0,state.hours)).padStart(2,"0")+":00";const progress=Math.round(((state.initialHours-state.hours)/state.initialHours)*100);$("stormProgress").style.width=progress+"%";$("progressLabel").textContent="MISSION PROGRESS • "+progress+"%";$("turn").textContent="TURN "+Math.min(state.turn,12)+" / 12";$("budget").textContent="₹"+state.budget;$("safety").textContent=Math.max(0,Math.round(state.safety))+"%";$("distance").textContent=Math.max(0,state.distance)+" km";$("confidence").textContent=Math.round(state.confidence)+"%";$("wind").textContent="WIND "+Math.round(state.wind)+" km/h • RAIN "+state.rainfall+" mm";$("phase").textContent=phase();
