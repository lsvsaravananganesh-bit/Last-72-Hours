...
const state = {
  ...
  forecastShift:0,landfallZone:"coastal",history:[],routesOpen:true,hospitalReady:52
};
...
function warning(){
  if(!spend(5))return;
  state.communications=Math.min(100,state.communications+12);state.confidence=Math.min(96,state.confidence+4);
...
function hospital(){
  if(!spend(12))return;
  state.medical=Math.min(100,state.medical+22);state.hospitalReady=Math.min(100,state.hospitalReady+25);
...
function roads(){
  if(!spend(8))return;
  state.vehicles=Math.min(24,state.vehicles+3);state.routesOpen=true;
...
function endGame(){
  ...
  $("reportText").textContent="Landfall has occurred. Your command protected an estimated "+people.toLocaleString("en-IN")+" of "+totalPopulation().toLocaleString("en-IN")+" residents. The final outcome reflects evacuation capacity, medical readiness, communications, route availability, resource limits and cascading events.";
...
$("restart").addEventListener("click",()=>{
  ...
  Object.assign(state,{hours:72,budget:100,safety:82,distance:720,confidence:61,wind:55,turn:1,ended:false,peopleProtected:0,evacuated:0,shelterCapacity:0,vehicles:18,teams:12,food:100,medical:100,communications:100,forecastShift:0,landfallZone:"coastal",history:[],routesOpen:true,hospitalReady:52});
