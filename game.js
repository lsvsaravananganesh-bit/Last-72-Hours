...
function startMission(){playTone("success");const overlay=$("briefingOverlay");if(overlay)overlay.classList.add("hidden");addFeed("BRIEFING • Mission started. You have 72 hours to prepare Surya Nagar.");}
function runQuickDemo(){if(state.ended)return;const sequence=["warning","shelter","evacuate","hospital","roads"];let i=0;const tick=()=>{if(i<sequence.length&&!state.ended){takeAction(sequence[i],"DEMO");i++;setTimeout(tick,550)}};tick();addFeed("JUDGE DEMO • Running the recommended 5-command sequence.");} 
...
function render(){
 ...
 const stormProgress=Math.min(1,(720-state.distance)/720),storm=$("storm");storm.style.right=(3+stormProgress*45)+"%";storm.style.bottom=(5+stormProgress*30)+"%";storm.style.transform="scale("+(1+stormProgress*.8)+")";
